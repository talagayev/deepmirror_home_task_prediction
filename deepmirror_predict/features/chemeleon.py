...
class CheMeleonConfig:
    ...
    - reduce_dim: optional deterministic projection after native embedding extraction
    - batch_size: number of molecules to featurize at once to avoid OOM
    - show_progress: prints simple batch progress
    """
...
def _deterministic_projection_matrix(in_dim: int, out_dim: int, seed: int = 0) -> np.ndarray:
    ...
    Produces a stable mapping across runs for reproducibility.
    """
...
class CheMeleonFingerprint:
    ...
    def _normalize_molecules(self, molecules: list[Union[str, Mol]]) -> list[Mol]:
        mols: list[Mol] = []
        for m in molecules:
            if isinstance(m, str):
                rm = MolFromSmiles(m)
    ...
    def _fingerprint_batch(self, mols: list[Mol]) -> np.ndarray:
    ...
    def __call__(self, molecules: list[Union[str, Mol]]) -> np.ndarray:
        mols = self._normalize_molecules(molecules)
        ...
        for start, mol_batch in _iter_batches(mols, self.cfg.batch_size):
            if self.cfg.show_progress:
                end = min(start + len(mol_batch), n_total)
                print(f"[CheMeleon] featurizing {start}:{end} / {n_total}")

            Xi = self._fingerprint_batch(mol_batch)
            blocks.append(Xi)
